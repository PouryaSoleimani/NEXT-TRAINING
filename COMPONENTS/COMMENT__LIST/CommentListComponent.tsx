    ...
    return (
        <div className='w-screen h-screen flex flex-wrap gap-10 p-20 items-center justify-around'>
            {commentsData.map((item: SingleCoinType) => (
                <Card key={item.id} className='size-32 flex items-center justify-center text-xl tracking-tighter font-bold bg-gradient-to-br from-violet-900 via-purple-900 to-blue-900 text-white'>{item.name.slice(0, 10)}</Card>
            ))}
        </div>
    )
}
