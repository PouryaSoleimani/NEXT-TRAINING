...
const SearchTestPage: NextPage<Props> = ({ products }) => {
  ...
  useEffect(() => {
    ...
    if (!resultArray.length) { setIsShowError(true) } else { setIsShowError(false) }
  ...
  return (
    <>
      <div className='bg-zinc-700 px-3 py-10 flex items-center justify-center'>
        <input type="text" placeholder='search' className='p-2 font-semibold w-72 text-black outline-none' value={search} onChange={searchHandler} />
      </div>
