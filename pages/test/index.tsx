...
const Index: NextPage<Props> = ({ data }) => {
  return (
    <section className='flex flex-col items-start justify-start'>
      ...
      <div className='bg-zinc-800 mt-4 w-fit rounded-xl mx-auto px-5 py-2 flex flex-wrap items-center justify-center  gap-10 space-y-3'>
        {data.map((item: SingleItemType) => (
          <Link href={`/test/${item.id}`} key={item.id}>
            <h1 className='text-xl w-[20rem] bg-slate-900 cursor-pointer font-semibold hover:bg-black p-2'>{item.id} - {item.title} : <span className='bg-green-600 text-green-950 font-extrabold text-xl px-3'>${item.price}</span></h1>
          </Link>
        ))}
      </div>
    </section>
