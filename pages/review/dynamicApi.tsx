...
const DynamicApi: NextPage<Props> = ({ allUsers }) => {
    ...
    return (
        <section id='FULL_PAGE_SECTION' className='w-screen h-screen pt-16 bg-zinc-300 flex flex-col space-y-10 items-center justify-center'>
            ...
            <form onSubmit={deleteUserHandler} className='border-2 border-zinc-800 w-1/2 h-1/3 p-10 flex flex-col items-center justify-start space-y-6'>
                <input className='px-4 py-2 text-black text-2xl border border-black outline-none font-bold rounded-xl w-1/2' type="text" placeholder="User ID you want to delete" value={ID} onChange={event => setID(event.target.value)} />
