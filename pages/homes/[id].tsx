...
const SingleHomePage: React.FC = () => {
 //STATES -----------------------------------------------------------------------------------------------------------------------
 type productProps = { id: number, title: string, view: string }
 ...
 const product = db.products.find((product) => { return product.id === id })
 ...
 return (
  <>
   ...
   <div>
    ...
    <ul className='flex flex-col justify-center items-start space-y-4'>
     ...
     {datas.map((product: productProps) => (
      <li className='text-3xl px-7 py-2 hover:bg-white hover:text-black cursor-pointer duration-500' key={product.id}> {product.title} -- {product.view}</li>
     ))}
   ...
   <div>
    ...
    <div>
     <h1 className='bg-white text-black p-6 font-bold text-3xl'> {product?.id} - {product?.title} - {product?.views}</h1>
    </div>
   </div>
