...
const DynamicLinksPage = () => {
    const courses = [
        ...
        { id: 2, title: 'NEXT', link: '/dynamic-links/next' },
        { id: 3, title: 'NODE', link: '/dynamic-links/node' },
        { id: 4, title: 'PWA', link: '/dynamic-links/pwa' },
        { id: 5, title: 'MONGODB', link: '/dynamic-links/mongodb' },
        { id: 6, title: 'EXPRESS', link: '/dynamic-links/express' },
    ]
    ...
    return (
        <div className='w-screen h-screen flex flex-col gap-y-10 mt-10 items-center juftify-center bg-black text-white px-10 overflow-x-hidden'>
            ...
            <ul className='w-[80%] flex flex-wrap gap-x-10 gap-y-10 justify-center items-center bg-zinc-900 p-10 border-2 border-zinc-600 rounded-2xl'>
                {courses.map(course => (
                    <li key={course.id} className='bg-zinc-700 no-underline text-white p-4 text-3xl font-bold rounded-xl hover:scale-105 hover:bg-yellow-500 duration-300'>
